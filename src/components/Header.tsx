...
const Header = ({
  ...
  const getRank = (bal: number) => {
    if (bal > 10000) return { label: 'WHALE', color: 'text-purple-400', bg: 'bg-purple-500/10' };
    if (bal > 5000) return { label: 'SHARK', color: 'text-blue-400', bg: 'bg-blue-500/10' };
    if (bal > 2000) return { label: 'DEGEN', color: 'text-emerald-400', bg: 'bg-emerald-500/10' };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 pl-2 pr-1 py-1 rounded-full shadow-inner">
            <div className={`px-2 py-0.5 rounded-full ${rank.bg} border border-current/20 flex items-center gap-1`}>
              ...
              <span className={`text-[8px] font-black ${rank.color}`}>{rank.label}</span>
            </div>
